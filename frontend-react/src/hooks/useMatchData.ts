...
// ✅ HELPER FUNCTION TO DETERMINE MATCH STATUS - TRUST BACKEND CLASSIFICATION
const determineMatchStatus = (backendMatch: any, sourceArray: 'live' | 'upcoming' | 'unknown' = 'unknown'): 'finalizada' | 'ao-vivo' | 'agendada' | 'adiada' | 'cancelada' => {
  // Handle explicit status mappings first
  ...
  // ✅ CRITICAL FIX: Trust the backend's classification
...
export const useLiveMatches = (): LiveMatchesReturn => {
  ...
  const fetchLiveMatches = useCallback(async () => {
    try {
      ...
      // ✅ DASHBOARD: Request limited to 6 matches for dashboard display
      ...
      const transformedMatches = liveMatches.map(match => transformBackendMatch(match, 'live'));
...
// ✅ HOOK FOR UPCOMING MATCHES - REAL DATA ONLY WITH 24H COUNT
export const useUpcomingMatches = (limit: number = 6): UpcomingMatchesReturn => {
  ...
  const fetchUpcomingMatches = useCallback(async () => {
    try {
      ...
      // ✅ FETCH LIMITED MATCHES FOR DISPLAY
      const limitedResponse = await fetch(
        `${API_BASE_URL}/api/v1/matches/search?status=upcoming&limit=${limit}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      // ✅ FETCH ALL UPCOMING MATCHES TO COUNT TOTAL IN 24H
      const allResponse = await fetch(
        `${API_BASE_URL}/api/v1/matches/search?status=upcoming&limit=1000`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!limitedResponse.ok || !allResponse.ok) {
        if (limitedResponse.status === 429 || allResponse.status === 429) {
          throw new Error('Rate limit exceeded. Please wait before refreshing.');
        }
      ...
      if (!limitedResult.success || !allResult.success) {
        throw new Error('Erro desconhecido ao buscar próximas partidas');
      }

      // ✅ PROCESS LIMITED MATCHES FOR DISPLAY
      const upcomingMatches = Array.isArray(limitedResult.data) ? limitedResult.data : (limitedResult.data?.matches || []);
      ...
      // ✅ CALCULATE TOTAL MATCHES IN NEXT 24 HOURS
      const allMatches = Array.isArray(allResult.data) ? allResult.data : (allResult.data?.matches || []);
      const now = Math.floor(Date.now() / 1000);
      const next24h = now + (24 * 60 * 60); // 24 hours from now

      const matchesIn24h = allMatches.filter((match: any) => {
        const matchTime = match.date_unix || 0;
        return matchTime > now && matchTime <= next24h;
      });

      console.log(`📊 Total upcoming matches in next 24h: ${matchesIn24h.length}`);

      setMatches(transformedMatches);
      setTotalIn24h(matchesIn24h.length);
      setLastUpdated(new Date());
