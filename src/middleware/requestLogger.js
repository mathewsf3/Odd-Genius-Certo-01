...
exports.httpLogger = exports.requestLogger = void 0;
...
const requestLogger = (req, res, next) => {
    ...
    logger_1.logger.info('Incoming request', Object.assign({ type: 'request_start' }, requestInfo));
    ...
    const logResponse = () => {
        ...
        if (statusCode >= 400 || environment_1.config.app.nodeEnv === 'development') {
        ...
        // Log the response
        logger_1.logger[logLevel]('Request completed', Object.assign({ type: 'request_complete' }, responseInfo));
        // Log performance metrics
        logger_1.logger.performanceMetric('request_duration', duration, 'ms', {
            method: req.method,
            path: req.path,
            statusCode,
        });
        ...
        if (statusCode === 401 || statusCode === 403) {
            logger_1.logger.securityEvent('unauthorized_access', 'medium', {
                requestId: req.requestId,
                ip: req.ip,
                userAgent: req.headers['user-agent'],
                path: req.path,
        ...
        if (req.path.startsWith('/api/')) {
            logger_1.logger.analyticsEvent('api_usage', {
                endpoint: req.path,
                method: req.method,
                statusCode,
                duration,
