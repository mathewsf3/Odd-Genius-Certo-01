...
export interface RefereePerformanceMetrics {
  ...
  strictnessRating: number; // 0-100, higher = more cards
  consistency: number; // 0-100
  controversyRating: number; // 0-100, based on unusual decisions
}

export interface PlayerComparison {
  player1: PlayerPerformanceMetrics;
  player2: PlayerPerformanceMetrics;
  comparison: {
    betterGoalScorer: number; // 1 or 2
    betterAssister: number;
...
export interface RefereeImpactAnalysis {
  referee: RefereePerformanceMetrics;
  impact: {
    homeAdvantageEffect: number; // How much referee affects home advantage
    gameFlowEffect: number; // How much referee affects game flow
    cardTendency: 'lenient' | 'strict' | 'average';
    goalTendency: 'high-scoring' | 'low-scoring' | 'average';
  };
  teamEffects: {
    teamId: number;
    teamName: string;
    winRateWithReferee: number;
    averageGoalsWithReferee: number;
    averageCardsWithReferee: number;
  }[];
}

export interface TopPerformersAnalysis {
  topScorers: PlayerPerformanceMetrics[];
  topAssisters: PlayerPerformanceMetrics[];
  mostConsistent: PlayerPerformanceMetrics[];
  highestImpact: PlayerPerformanceMetrics[];
  risingStars: PlayerPerformanceMetrics[];
  veteranPerformers: PlayerPerformanceMetrics[];
}

export class PlayerAnalyticsHelpers {
  ...
  static calculatePlayerPerformance(
    ...
    const averageRating = playerStats?.averageRating || 0;
   ...
   */
  static calculateRefereePerformance(
    referee: Referee,
    matches: Match[],
    refereeStats?: any
    ...
    const totalCards = refereeMatches.reduce((sum, match) => 
      sum + (match.team_a_red_cards || 0) + (match.team_b_red_cards || 0), 0
    );
    const totalGoals = refereeMatches.reduce((sum, match) => 
      sum + match.homeGoalCount + match.awayGoalCount, 0
    );

    let homeWins = 0, awayWins = 0, draws = 0;
    refereeMatches.forEach(match => {
      if (match.homeGoalCount > match.awayGoalCount) homeWins++;
      else if (match.awayGoalCount > match.homeGoalCount) awayWins++;
      else draws++;
    });
   ...
   */
  static comparePlayers(
    player1Metrics: PlayerPerformanceMetrics,
    player2Metrics: PlayerPerformanceMetrics
  ): PlayerComparison {
    const comparison = {
    ...
    const player1Score = 
      (comparison.betterGoalScorer === 1 ? 1 : 0) +
   ...
   */
  static analyzeRefereeImpact(
    refereeMetrics: RefereePerformanceMetrics,
    matches: Match[],
    teams: any[]
  ): RefereeImpactAnalysis {
    const refereeMatches = matches.filter(match =>
      match.refereeID === refereeMetrics.refereeId
    );
  ...
  private static calculatePlayerForm(matches: Match[], playerId: number): string {
    // Simplified form calculation - would need detailed match data
    // For now, return a placeholder
    return 'WWDLW';
  }

  private static calculatePlayerConsistency(playerStats: any): number {
    // Calculate consistency based on performance variation
    // Simplified calculation
    if (!playerStats || !playerStats.matchRatings) return 50;
    
    const ratings = playerStats.matchRatings;
    const stdDev = AnalyticsUtils.calculateStandardDeviation(ratings);
    const avgRating = AnalyticsUtils.calculateAverage(ratings);
  ...
  private static getEmptyRefereeMetrics(referee: Referee): RefereePerformanceMetrics {
    return {
      refereeId: referee.id,
      refereeName: referee.name,
      matchesOfficiated: 0,
  ...
  private static calculateRefereeStrictness(averageCardsPerGame: number): number {
  ...
  private static calculateRefereeConsistency(matches: Match[]): number {
    ...
    const stdDev = AnalyticsUtils.calculateStandardDeviation(cardsPerMatch);
    const avgCards = AnalyticsUtils.calculateAverage(cardsPerMatch);
  ...
  private static calculateRefereeControversy(matches: Match[]): number {
    ...
    matches.forEach(match => {
      ...
      if (totalCards > 4) controversyScore += 10; // High card count
      
      const goalDifference = Math.abs(match.homeGoalCount - match.awayGoalCount);
      if (goalDifference > 3) controversyScore += 5; // Unusual scoreline
    });

    return Math.min(100, controversyScore);
  }

  private static calculateHomeAdvantageEffect(matches: Match[]): number {
  ...
  private static calculateGameFlowEffect(matches: Match[]): number {
    if (matches.length === 0) return 0;
  ...
  private static determineCardTendency(averageCards: number): 'lenient' | 'strict' | 'average' {
    if (averageCards < 2) return 'lenient';
    if (averageCards > 4) return 'strict';
    return 'average';
  }

  private static determineGoalTendency(averageGoals: number): 'high-scoring' | 'low-scoring' | 'average' {
    if (averageGoals < 2.2) return 'low-scoring';
    if (averageGoals > 3.2) return 'high-scoring';
    return 'average';
  }

  private static calculateTeamEffects(matches: Match[], teams: any[]) {
    const teamEffects: any[] = [];
