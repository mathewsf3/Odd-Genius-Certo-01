...
export class AnalyticsUtils {
  ...
  static calculateTeamForm(matches: Match[], teamId: number, lastN: number = 5): string {
  ...
  static calculateGoalAverage(matches: Match[], teamId: number): { for: number; against: number } {
    const teamMatches = matches.filter(match => 
      match.homeID === teamId || match.awayID === teamId
    );
    
    if (teamMatches.length === 0) return { for: 0, against: 0 };
  ...
  static calculateBTTSPercentage(matches: Match[]): number {
  ...
  static calculateOver25Percentage(matches: Match[]): number {
    if (matches.length === 0) return 0;
    
    const over25Matches = matches.filter(match => 
  ...
  static calculateCleanSheetPercentage(matches: Match[], teamId: number): number {
    const teamMatches = matches.filter(match => 
      match.homeID === teamId || match.awayID === teamId
    );
  ...
  static calculateTrendStrength(values: number[]): number {
  ...
  static calculateMatchProbabilities(homeGoalExpectancy: number, awayGoalExpectancy: number): {
    homeWin: number;
    draw: number;
    awayWin: number;
  } {
    ...
    for (let homeGoals = 0; homeGoals <= 5; homeGoals++) {
      for (let awayGoals = 0; awayGoals <= 5; awayGoals++) {
        const probability = 
          this.calculatePoissonProbability(homeGoalExpectancy, homeGoals) *
  ...
  static groupMatchesByDate(matches: Match[]): Map<string, Match[]> {
  ...
  static groupMatchesByTeam(matches: Match[]): Map<number, Match[]> {
  ...
  static filterMatchesByDateRange(matches: Match[], startDate: Date, endDate: Date): Match[] {
  ...
  static calculateEloRating(currentRating: number, opponentRating: number, result: number, kFactor: number = 32): number {
  ...
  static calculateTeamStrength(matches: Match[], teamId: number): number {
    const teamMatches = matches.filter(match => 
      match.homeID === teamId || match.awayID === teamId
    );
    
    if (teamMatches.length === 0) return 1000; // Default Elo rating
    ...
    teamMatches.forEach(match => {
      ...
      let result = 0.5; // Draw
      if (teamGoals > opponentGoals) result = 1; // Win
      if (teamGoals < opponentGoals) result = 0; // Loss
