...
class LeagueAnalyticsHelpers {
    ...
    static calculateLeagueStatistics(matches) {
        ...
        completedMatches.forEach(match => {
            ...
            if (totalMatchGoals > 2.5)
                over25Count++;
            if (totalMatchGoals > 3.5)
                over35Count++;
            if (homeGoals === 0 || awayGoals === 0)
                cleanSheetCount++;
            if (totalMatchGoals >= 4)
                highScoringCount++;
     ...
     */
    static generateLeagueTable(matches, teams) {
        ...
        const table = Array.from(teamStats.values()).map(team => ({
     ...
     */
    static analyzeSeasonTrends(matches, teams, season) {
    ...
    static compareCompetitions(competitions) {
        ...
        // Determine rankings
        const rankings = {
            mostCompetitive: leagues.reduce((prev, curr) => curr.competitiveness > prev.competitiveness ? curr : prev).leagueName,
            highestScoring: leagues.reduce((prev, curr) => curr.statistics.averageGoalsPerMatch > prev.statistics.averageGoalsPerMatch ? curr : prev).leagueName,
            mostDefensive: leagues.reduce((prev, curr) => curr.statistics.averageGoalsPerMatch < prev.statistics.averageGoalsPerMatch ? curr : prev).leagueName,
            mostPredictable: leagues.reduce((prev, curr) => curr.statistics.homeWinPercentage > prev.statistics.homeWinPercentage ? curr : prev).leagueName
        };
     ...
     */
    static getEmptyLeagueStatistics() {
        return {
            totalMatches: 0,
            completedMatches: 0,
            totalGoals: 0,
    ...
    static calculateSeasonTrends(matches, table) {
        ...
        const weeklyAverages = goalsByWeek.map(week => week.reduce((sum, match) => sum + match.homeGoalCount + match.awayGoalCount, 0) / week.length);
        const goalTrend = AnalyticsUtils_1.AnalyticsUtils.calculateTrend(weeklyAverages);
        ...
        const homeWinRate = matches.filter(match => match.status === 'complete' && match.homeGoalCount > match.awayGoalCount).length / matches.filter(match => match.status === 'complete').length;
    ...
    static groupMatchesByWeek(matches) {
        const weeks = [];
        let currentWeek = [];
    ...
    static calculateCompetitiveness(table) {
        if (table.length < 2)
            return 0;
        ...
        const avgPoints = AnalyticsUtils_1.AnalyticsUtils.calculateAverage(points);
        const stdDev = AnalyticsUtils_1.AnalyticsUtils.calculateStandardDeviation(points);
    ...
    static calculateLeagueQuality(statistics) {
...
exports.LeagueAnalyticsHelpers = LeagueAnalyticsHelpers;
