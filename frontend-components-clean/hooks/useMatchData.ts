...
export const useLiveMatches = (): UseMatchDataReturn => {
  ...
  const fetchLiveMatches = useCallback(async () => {
    try {
      ...
      // ✅ SET REAL DATA - NO FALLBACKS - ENHANCED SCORE HANDLING
      // Transform data to ensure proper score handling
      const transformedMatches = result.data.map((match: any) => ({
        ...match,
        placarCasa: match.homeGoalCount != null ? match.homeGoalCount : 0,
        placarVisitante: match.awayGoalCount != null ? match.awayGoalCount : 0
      }));
